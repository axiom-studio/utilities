...
if [[ $# -lt 2 ]]; then
  echo "usage: $0 <service_account_name> <namespace> [--axiom-endpoint=<value>] [--axiom-api-token=<value>] [--cluster-name=<value>] [--insecure=<value>] [--server_url=<value>]"
  echo "ex: sh ./kubeconfig-exporter/kubernetes_export_sa.sh cd-user axiomcd"
  exit 1
fi
...
CLIENT_VERSION=$(echo "${CLIENT_VERSION%+}")
echo "$CLIENT_VERSION"
...
if [[ ! -z "${axiom_endpoint}" ]] && [[ ! -z "${axiom_api_token}" ]] && [[ ! -z "$cluster_name" ]]; then
    ...
    res=$(curl -k "${axiom_endpoint}/orchestrator/cluster" \
        -H "Content-Type: application/json" \
        -H "token: $axiom_api_token" \
        --data-raw "$json_data")
...
elif ([ ! -z "${axiom_endpoint}" ] && [  -z "${axiom_api_token}" ] && [  -z "$cluster_name" ]) || \
     ([  -z "${axiom_endpoint}" ] && [ ! -z "${axiom_api_token}" ] && [  -z "$cluster_name" ]) || \
     ([  -z "${axiom_endpoint}" ] && [ -z "${axiom_api_token}" ] && [ ! -z "$cluster_name" ]) || \
     ([ ! -z "${axiom_endpoint}" ] && [ ! -z "${axiom_api_token}" ] && [  -z "$cluster_name" ]) || \
     ([ ! -z "${axiom_endpoint}" ] && [  -z "${axiom_api_token}" ] && [ ! -z "$cluster_name" ]) || \
     ([  -z "${axiom_endpoint}" ] && [ ! -z "${axiom_api_token}" ] && [ ! -z "$cluster_name" ]); then

    echo "Please provide all required inputs to add cluster on axiom studio"\
