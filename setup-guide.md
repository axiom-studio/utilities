# Axiom Cluster Setup Guide

This guide provides a standardized approach for provisioning a **dedicated Kubernetes cluster for Axiom**, following best practices for reliability, security, and cost-efficiency. Axiom should be deployed in an **isolated cluster**, separate from production workloads, to maintain a clean separation of responsibilities.

---

## Table of Contents
- [Prerequisites (Applicable to All Cloud Providers)](#prerequisites-applicable-to-all-cloud-providers)
- [Access for Axiom Team (If Using Managed Deployment)](#access-for-axiom-team-if-using-managed-deployment)
...
- Axiom must run in its own Kubernetes cluster (not shared with app workloads).
- Supports any CNCF-compliant cluster: EKS, GKE, AKS, self-hosted, etc.

### 2. Node Group Configuration
Provision **two separate node groups**:

#### a. `axiom-workloads` (On-Demand Instances)
- Runs Axiom's core services and Axiom-related workloads.
...
> Note: These labels and taints typically will be used after the Axiom setup. For more details, refer to the [Axiom Build Infra documentation](https://docs.axiom.co/global-configurations/build-infra).

###  3. Object Storage Buckets

| Bucket Name                  | Purpose                                 | Retention Recommendation           | Required/Optional |
|------------------------------|-----------------------------------------|------------------------------------|-------------------|
| axiom-ci-logs              | CI logs                                 | -                           | -          |
| axiom-ci-cache             | CI cache artifacts                      | 7–30 days                         | Required          |
| axiom-backup               | Backups                                 | 10-30 days                        | Required          |
| axiom-microservice-logs    | Axiom microservice logs (audit/logs)  | 3–12 months (per audit needs)      | Optional          |
...
* Configure **lifecycle rules** or **retention policies** on the CI cache bucket, backups, and Axiom microservice logs bucket to:
...
## Access for Axiom Team (If Using Managed Deployment)

If Axiom Team is managing the cluster setup and lifecycle:

* Provide a **Kubernetes `cd-user` service account** with the required RBAC permissions for Axiom installation and management. For step-by-step instructions on generating a secure cluster-admin token and exporting kubeconfig for this service account, refer to the [kubeconfig-exporter documentation](https://github.com/axiom-studio/utilities/tree/main/kubeconfig-exporter#script-to-generate-cluster-admin-tokens).
* Share a **cluster-admin level token** or access credentials securely.
* Axiom Team will manage installation, upgrades, monitoring, and integrations.
...
attachPolicyARNs:
  ...
  - arn:aws:iam::<your-account-id>:policy/axiom-cluster-IAM-policy 
```
**Optional (Recommended for Least Privilege):**
If you are following **[Option 5 (Optional: Custom IAM Policy for axiom-cluster-IAM-policy)](#5-optional-custom-iam-policy-for-axiom-cluster-iam-policy)**, you can attach only the required custom policies (S3, ECR, ELB) instead of full admin-level policies. This improves security posture by granting only the necessary permissions. See [Option 5 details below](#5-optional-custom-iam-policy-for-axiom-cluster-iam-policy) for how to create and use these policies.
...
#### 5. Optional: Custom IAM Policy for axiom-cluster-IAM-policy 
...
---
## Support
For onboarding assistance, cluster validation, or enterprise SLAs, please contact the Axiom team via your assigned solutions engineers.